...
from graph_pool_custom import GraphMultisetTransformer_custom
from torch_geometric.utils import to_dense_adj

#from onmt.decoders import TransformerDecoder
from transformer_mod import TransformerDecoder, TransformerLMDecoder
#from onmt.modules.embeddings import Embeddings
from embeddings_mod import Embeddings
...
class GraphEncoder(nn.Module):
    def __init__(self, node_dim, edge_dim, hidden_dim, device, model_config):
        ...
        self.tconv1 = TammoConv(node_dim, edge_dim, hidden_dim, device)
        
        self.mu = TammoConv(node_dim, edge_dim, hidden_dim, device, first_linear=False)
        self.logvar = TammoConv(node_dim, edge_dim, hidden_dim, device, first_linear=False)
...
class GraphEncoder_GMT(nn.Module):
    def __init__(self, node_dim, edge_dim, hidden_dim, device, model_config):
        super(GraphEncoder_GMT, self).__init__()
        ...
        self.n_nodes_pool = model_config['n_nodes_pool']
        self.pool_mu = GraphMultisetTransformer_custom(hidden_dim, k=self.n_nodes_pool)
        self.pool_var = GraphMultisetTransformer_custom(hidden_dim, k=self.n_nodes_pool)
        # custom GMT clusters to k nodes which are concatenated-> we then compress again to 
        self.bridge_layer_mu = Sequential(Linear(int(hidden_dim*self.n_nodes_pool), hidden_dim),ReLU(),).to(device)
        self.bridge_layer_var = Sequential(Linear(int(hidden_dim*self.n_nodes_pool), hidden_dim),ReLU(),).to(device)
    ...
    def forward(self, graph, dest_is_origin_matrix, inc_edges_to_atom_matrix, device):
        ...
        #mu = global_mean_pool(mu * atom_weights.view(-1, 1), graph.batch).to(device)
        #logvar = global_mean_pool(logvar * atom_weights.view(-1, 1), graph.batch).to(device)
        mu = self.pool_mu(shared_output, graph.batch).to(device)
        mu = self.bridge_layer_mu(mu)
        logvar = self.pool_mu(shared_output, graph.batch).to(device)
        logvar = self.bridge_layer_var(logvar)
        # stack in case of GMT pooling
        #mu = torch.cat()
        #print(torch.mean(mu), torch.mean(logvar))
...
class G2S_Gab(nn.Module):
    ...
    def sample(self, mean, log_var, eps_scale=0.01):
        ...
        if self.training:
            ...
            eps = torch.randn_like(std) * eps_scale  # TODO: not drawn from standard distribution? We should change this to make sure std is not involved in sampling?
    ...
    def forward(self, batch_list, dest_is_origin_matrix, inc_edges_to_atom_matrix, device):
        ...
        z = self.sample(h_G_mean, h_G_var)
...
class G2S_PPguided(nn.Module):
    ...
    def forward(self, batch_list, dest_is_origin_matrix, inc_edges_to_atom_matrix, device):
        ...
        mse = self.mse_loss(y_true,y)

        # decode
        recon_loss, acc, predictions, target = self.Decoder(batch_list, z)

        return recon_loss + self.beta*kl_loss + self.alpha*mse, recon_loss, kl_loss, acc, predictions, target, z
