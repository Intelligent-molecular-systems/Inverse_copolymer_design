...
def make_monomer_mols(smiles: str, keep_h: bool, add_h: bool, fragment_weights: list):
    """
    Builds an RDKit molecule from a SMILES string.
    ...
    num_frags = len(smiles.split('.'))
    if len(fragment_weights) != num_frags:
        print(f'number of input monomers/fragments ({num_frags}) does not match number of '
                         f'input number of weights ({len(fragment_weights)})')
