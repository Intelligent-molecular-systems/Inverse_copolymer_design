...
if os.path.isfile(filepath):
    ...
    # only for first batch
        model.eval()
        for i in range(250):
            z_rand = torch.randn((64,embedding_dimension), device=device)*0.01
            predictions_rand, _, _, z = model.inference(data=z_rand, device=device, sample=False, log_var=None)
    ...
    # only for first batch
        ...
        _, _, _, z = model.inference(data=data, device=device, dest_is_origin_matrix=dest_is_origin_matrix, inc_edges_to_atom_matrix=inc_edges_to_atom_matrix, sample=False, log_var=None)
        ...
        for r in range(8):
            ...
            predictions_seed, _, _, z = model.inference(data=seed_z_noise, device=device, sample=True, log_var=None)
    ...
    # only for first batch
        ...
        for e in range(examples):
            ...
            for s in range(interpolated_vectors.shape[0]):
                prediction_interp, _, _, _ = model.inference(data=interpolated_vectors[s], device=device, sample=True, log_var=None)
