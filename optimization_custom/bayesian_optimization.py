...
class BayesianOptimization(Observable):
    ...
    def maximize(self,
        ...
        # Cutom modification to use max_time
        start_time = time.time()
        elapsed_time = 0
        iteration = 0
        if max_time is None: # Use the n_iter criterion to terminate the optimization
        ...
        else: # Use the max time criterion to terminate the optimization
            while not self._queue.empty or elapsed_time < max_time:
                try:
                    x_probe = next(self._queue)
                except StopIteration:
                    util.update_params()
