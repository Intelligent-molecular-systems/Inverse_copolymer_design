...
# Path to your subdirectory with text files
path = os.path.join(os.getcwd(), 'plotting/results/top20_mols_GA_correct_*.txt')
import glob
# Loop through each file in the directory
for filename in glob.glob(path):
    # Extract information from filename using regex
    match = re.search(r"top20_mols_GA_correct_([A-Za-z0-9]+)_(time|iter)_(\d+)_run(\d+)", filename)
    if match:
        objective_type = match.group(1)     # Could be any string (EAmin, or other)
        time_or_iter = match.group(2)    # "time" or "iter"
        time_iter_value = match.group(3)      # Numeric value for time/iter
        run_number = match.group(4)      # Run number


    file_path = os.path.join(os.getcwd(), 'plotting/results/', filename)
        ...
        for iteration, poly_string in mols_dict.items():
            if not poly_string in poly_strings:
                ...
                if objective_type=='mimick_peak':
                    val_obj = (abs(val_EA + 2.0) + abs(val_IP - 1.2))
    ...
    plt.savefig('plotting/results/optimal_mols_GA_correct_.png', bbox_inches='tight', pad_inches=0.1, dpi=300)
    import json
