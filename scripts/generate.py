...
if os.path.isfile(filepath):
    ...
    # only for first batch
        ...
        for r in range(8):
            ...
            predictions_seed, _, _, z, y = model.inference(data=seed_z_noise, device=device, sample=True, log_var=None)
    ...
    # only for first batch
        ...
        for e in range(examples):
            ...
            for s in range(interpolated_vectors.shape[0]):
                prediction_interp, _, _, _, y = model.inference(data=interpolated_vectors[s], device=device, sample=True, log_var=None)
