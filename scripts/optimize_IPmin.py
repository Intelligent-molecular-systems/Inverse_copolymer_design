...
class Property_optimization_problem(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        out["F"][~invalid_mask, 1] = -self.weight_ionization_potential * expanded_y_p[~invalid_mask, 1]  # Bring the second property (ionization potential) as close to 1 as possible
...
with open(dir_name+'res_optimizationIPmin', 'wb') as f:
    pickle.dump(res, f)
with open(dir_name+'best_solutionsIPmin', 'wb') as f:
...
with open(dir_name+'prediction_stringsIPmin', 'wb') as f:
    pickle.dump(prediction_strings, f)
with open(dir_name+'predicted_yIPmin', 'wb') as f:
    pickle.dump(predicted_y, f)

#with torch.no_grad():
#    best_solution = torch.from_numpy(best_solution).to(device).to(torch.float32) 
#    predictions, _, _, _, y = model.inference(data=best_solution, device=device, sample=False, log_var=None)
#    prediction_strings = [combine_tokens(tokenids_to_vocab(predictions[sample][0].tolist(), vocab), tokenization=tokenization) for sample in range(len(predictions))]

#print(prediction_strings, convergence.converged_solution_F)

#with torch.no_grad():
#    best_solution = torch.from_numpy(convergence.converged_solution_X).to(device).to(torch.float32) 
#    predictions, _, _, _, y = model.inference(data=best_solution, device=device, sample=False, log_var=None)
#    prediction_strings = [combine_tokens(tokenids_to_vocab(predictions[sample][0].tolist(), vocab), tokenization=tokenization) for sample in range(len(predictions))]

#print(convergence.converged_solution_X, convergence.converged_solution_F)
#print(res.X, res.F)
#print(best_solution, y)

# Take the best 5 molecules and encode them again+ compare the latent space/prediction/decoded molecules
...
torch.save(dict_results_loader, dir_name+'/dict_results_loaderIPmin.pt')
...
with open(dir_name+'res_optimizationIPmin', 'rb') as f:
    res = pickle.load(f)
with open(dir_name+'best_solutionsIPmin', 'rb') as f:
    best_solutions = pickle.load(f)
with open(dir_name+'prediction_stringsIPmin', 'rb') as f:
    prediction_strings = pickle.load(f)
with open(dir_name+'predicted_yIPmin', 'rb') as f:
    predicted_y = pickle.load(f)
dict_results_loader = torch.load(dir_name+'/dict_results_loaderIPmin.pt')
...
i=0
with open(dir_name+'results_optimizationIPmin.txt', 'w') as f:
...
plt.legend()
plt.savefig(dir_name+'paretofrontIPmin.png')
