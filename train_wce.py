...
tokenization = "oldtok" #oldtok or RT_tokenized
...
model = G2S_Gab(num_node_features,num_edge_features,hidden_dimension,embedding_dim,device,model_config, vocab, loss_weights=class_weights)
model.to(device)
print(model)
...
def weights_init_xavieruniform(m):
    classname = m.__class__.__name__
    # for every Linear layer in a model..
    if classname.find('Linear') != -1:
...
n_iter = int(80/4 * num_train_graphs/batch_size)
...
model_name = 'Model_h='+str(model_config['hidden_dimension'])+'_decL='+str(model_config['decoder_num_layers'])+'_nheads='+str(model_config['num_attention_heads'])+'_beta='+str(model_config['beta'])+'_loss='+str(model_config['loss'])+'_augmented_oldtok_randomseed_newVAE_2/'
