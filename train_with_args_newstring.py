...
dict_train_loader = torch.load('dict_train_loader_'+augment+'_'+tokenization+'_gbigsmileslike.pt')
dict_val_loader = torch.load('dict_val_loader_'+augment+'_'+tokenization+'_gbigsmileslike.pt')
dict_test_loader = torch.load('dict_test_loader_'+augment+'_'+tokenization+'_gbigsmileslike.pt')
...
# Initialize model
model = G2S_Gab(num_node_features,num_edge_features,hidden_dimension,embedding_dim,device,model_config, vocab, seed, loss_weights=class_weights, add_latent=add_latent)
model.to(device)
print(model)
...
if args.initialization == "xavier":
    model.apply(weights_init_xaviernormal)


n_iter = int(20 * num_train_graphs/batch_size)# 20 epochs
# Beta scheduling function from Optimus paper 
def frange_cycle_zero_linear(n_iter, start=0.0, stop=1.0,  n_cycle=5, ratio_increase=0.5, ratio_zero=0.3): #, beginning_zero=0.1):
...
model_name = 'Model_beta='+str(args.beta)+'_loss='+str(args.loss)+'_augment='+str(args.augment)+'_tokenization='+str(args.tokenization)+'_AE_warmup='+str(args.AE_Warmup)+'_init='+str(args.initialization)+'_seed='+str(args.seed)+'_add_latent='+str(add_latent)+'_gbigsmileslike/'
directory_path = os.path.join(os.getcwd(),'Checkpoints_new/', model_name)
